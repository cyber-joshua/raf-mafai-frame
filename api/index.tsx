...
const getPrize = tool((role) => {
  return `$RAF-${role}`;
}, {
  name: 'get_nft',
  description: 'Tells user what kind of NFT the user can mint after the user gets his/her giraffe role',
})
...
app.frame('/', async (c) => {
  ...
  const state = await deriveState(async previousState => {
    if (minted) {
      if (!previousState.gameStarted) {
        previousState.agentMsg = 'Don\'t leave yet! Something happened!! Solve the "The Raf Syndicate’s Betrayal" mystery to get more prizes!!!';
        previousState.threadId = new Date().getTime().toString();
      } 
      if (buttonValue === 'startGame' || inputText) {
        try {
          const humanMessage = buttonValue === 'startGame' ? 
            new HumanMessage(previousState.giraffeRole ? `I am ${previousState.giraffeRole}, what happened?` : "Ok, Assign me a role and tell me what happened?") : 
  ...
  return c.res({
    ...
    intents: [
      ...
      state.win && !minted && <Button.Transaction target="/mint">Mint Your RAF</Button.Transaction>,
      minted && !state.gameStarted && <Button value="startGame">Start Game</Button>,
      minted && state.prize && <Button.Transaction target="/claim">Claim Prize</Button.Transaction>
    ],
  })
})
